...
void UUIMainWidget::SetMpPersent()
{
	...
	FText text = FText::Format(LOCTEXT("CharacterState", "MP({current} / {max}"), pCharacter->CurMp, pCharacter->MaxMp);
	Text_MP->SetText(text);

	ProgressBar_MP->SetPercent(pCharacter->CurMp / pCharacter->MaxMp);
}
